...
void mexFunction(int nout, mxArray *pout[], int nin, const mxArray *pin[]) {
	...
	int DIMS[] = {aw, ah};
	pout[0] = mxCreateNumericArray(2, DIMS, mxINT32_CLASS, mxREAL);
	pout[1] = mxCreateNumericArray(2, DIMS, mxDOUBLE_CLASS, mxREAL);
	int *nnf_XY = (int *)mxGetData(plhs[0]); 
	double *nnf_dist = (double *)mxGetData(plhs[1]);
	
	for(int i=0;i<aw*ah;i++)
		nnf_dist[i] = 1e20;

	double *int_win_pad = double[(aw+1)*(ah+1)];
