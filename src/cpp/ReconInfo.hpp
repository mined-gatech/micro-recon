...
class ReconInfo
{
    public:
        ...
        ReconInfo(const mxArray * Recon)
        {
            ...
            mxArray *mxAngMap = mxGetField(Recon, 0, "AngMap");
            if(mxAngMap != NULL)
            {
            ...
            NumExemplars = mxGetNumberOfElements(getField(Recon, "E"));
            ...
            for(int i=0; i<NumExemplars; i++) {
                ...
                mxArray *E = mxGetCell(getField(Recon, "E"), i);
                ...
                // Get the exemplar short vector 2-pt statistics
                mxArray * stats = 
                    mxGetCell(getField(Recon, "Exemplar_Stats"), i);
                mxArray * statsNorm = 
                    mxGetCell(getField(Recon, "Exemplar_Stats_Norm"), i);
                ...
                Exemplars.push_back(
                    new Exemplar(data, numComponents, NB_SIZE, NBLookup,
                        szX, szY, nbOffsets, mxGetPr(txW), mxGetPr(stats), 
                        mxGetPr(statsNorm)));
            ...
            /*
            // Allocate arrays to keep track of the current statistics.
            // Copy from the matab recon object if needed.
            for(int i=0; i<NumExemplars; i++) {
                double *s = mxGetPr(mxGetCell(getField(Recon, "CurrentStats"), i));
                double *sN = mxGetPr(mxGetCell(getField(Recon, "CurrentStatsNorm"), i));
                Stats.push_back(new double[NB_SIZE*NB_SIZE]);
                StatsNorm.push_back(new double[NB_SIZE*NB_SIZE]);
                memcpy((void *)Stats[i], (void *)s, sizeof(double)*NB_SIZE*NB_SIZE);
                memcpy((void *)StatsNorm[i], (void *)sN, sizeof(double)*NB_SIZE*NB_SIZE);
            }
            */
        ...
        // Get the current statistics of the reconstruciton
        double * getCurrentStats() { return Stats[0]; }
        ...
        // A copy of the statistics and normalizations for current stats
        // These will be NB_SIZExNB_SIZE so no need to keep dimensions
        vector<double *> Stats;
